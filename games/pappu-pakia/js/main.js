...
mit.main = function() {
  ...
  */
  ...
  tweet.href='http://twitter.com/share?url=http://www.zufalplay.com/games/pappu-pakia/&text=I am playing Pappu Pakia, a game on www.zufalplay.com!&count=horiztonal&via=zufalplay';

  var facebook = document.getElementById("fb");
  facebook.href='https://www.facebook.com/zufalplay';
  ...
  mit.gameOver = function() {
    ...
    if(addscore_called==0) 
    {
       ...
       $.ajax(
      {
        ...
        {
          mode:'encrypt_score',
          this_score:Math.floor(mit.score),
        },
      ...
      var score_display = Math.floor(mit.score);
      setTimeout(function(){ window.location.replace("../newgame.php?game=pappupakia&score="+score_display);}, 2000);
    }
    ...
    // Share
    var tweet = document.getElementById("tweet");
    tweet.href='http://twitter.com/share?url=http://www.zufalplay.com/&text=I just scored ' +Math.floor(mit.score)+ ' points in Pappu Pakia!&count=horiztonal&via=zufalplay';
  ...
  (function renderGame() {
    ...
    if (mit.game_started) {
      ...
      if (!mit.game_over) {
        ...
        if(Math.floor(mit.score) % 100 == 0)
        {
          var zufals_profit = Math.min(10*(mit.score/5000)*(mit.score/5000),10);
          $('.progress-bar').css('width', zufals_profit*10 +'%').attr('aria-valuenow','');
